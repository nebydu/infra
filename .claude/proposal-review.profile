...
#   벗어난 곳에 profile을 두면 그 위치 기준으로 해석된다.
...
PROPOSAL_REVIEW_POLICY="infra는 Kafka/OTel Collector docker compose 설정 repo다(빌드/테스트 없음 — 검증 기준은 docker compose config 문법과 설정 일관성). 결정 리뷰 시 지킬 기준: 방향 판단의 최상위 기준은 통합본(이 입력에 미포함, 170KB라 제외 — ../monitoring-meta/docs/master-design.md)이고, 데모 spec v0.2.1은 Phase 0 회귀 방지 가드다. 단 ADR-0005 토픽 재명명은 Phase 1 forward 변경이지 회귀가 아니다. Accepted ADR(0002 heartbeat otlp_proto — hub 디코더와 동시 컷오버 전제, 0005 토픽 명명 규칙 B)과 충돌하는 제안, [Open]/미결정 ADR을 결정된 것으로 전제한 제안, 형제 repo(hub/script-agent/monitoring-meta)를 infra가 직접 수정하는 것을 전제한 제안은 block 대상이다. 작업 spec은 ../monitoring-meta/handoff/<work-id>/<work-id>-infra.md 경유가 원칙이므로 이를 우회하는 프로세스 제안도 결함으로 지적하라. 통합본이 직접 쟁점인데 발췌가 없으면 missing_context로 지적하라."
