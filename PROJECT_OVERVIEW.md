...
> **이 한 장이 무엇인가.** 모니터링 솔루션 데모 monorepo(`monitoring/`)의 현재
> 상태를 빠짐없이 전달하기 위한 **단일 컨텍스트 문서**. 외부 LLM/리뷰어에게
> 이 파일 하나만 던져도 다음을 모두 파악할 수 있다:
>
> - 워크스페이스 구조 (hub + script-agent + infra)
> - 모듈별 책임/기능/엔드포인트
...
> - 종단 검증 시나리오 / 빌드·실행 명령
>
> 작성 기준일: **2026-05-23**, spec 버전 **v0.2.1**. 코드 변경 시 본 문서의
...
├── hub/                         # Spring Boot BE (JDK 21 / Maven)
│   ├── README.md
...
### git 진행 상태 (2026-05-23 기준)

```
134bba4 fix(agent): commands 처리 at-least-once 보장 강화
d10beb5 Kafka 역직렬화 설정 검증 추가
fe3b799 feat(hub): 데모 BE walking skeleton 완성 — 양방향 통신 + Thymeleaf UI
91bccff feat: monitoring 데모 monorepo 초기 커밋 — Script Agent v0.1 + 공유 인프라
```
...
- 실제 코드 변경을 다룬다면: `hub/src/`, `script-agent/internal/`, `script-agent/cmd/agent/`
- 인프라 변경을 다룬다면: `infra/docker-compose.yml`, `infra/otel-collector-config.yml`
- ADR/spec 정밀 검토라면: `hub/docs/monitoring-demo-message-spec-v0.2.1.md` 원본
- 최신 진행 상태: `git log --oneline -20`
